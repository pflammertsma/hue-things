package com.pixplicity.huethings.dht;
...
public class Dhtxx implements AutoCloseable {
    ...
    private long mLastReadTime = -MIN_INTERVAL;
    ...
    private synchronized boolean readSample(boolean force)
     ...
     * @throws IOException
     */
    private synchronized boolean readSampleForced()
            throws IOException, IllegalStateException, InterruptedException {
        ...
        if (true) {
            ...
            long endTime = System.nanoTime() + 100 * 1000000;
            int lineLength = 0;
            while (System.nanoTime() < endTime) {
                ...
                sb.append(mGpio.getValue() ? 1 : 0);
                lineLength--;
            }
            Log.d(TAG, "readSampleForced: " + sb.toString());
        ...
        for (int i = 0; i < cycles.length; i += 2) {
            ...
            expectPulse(false, 50);
            cycles[i] = expectPulse(true, 30) ? 1 : 0;
            cycles[i + 1] = expectPulse(true, 30) ? 1 : 0;
        }
    ...
    private synchronized boolean expectPulse(boolean level, int durationMicroseconds)
            throws IOException {
        boolean pulse = false;
        long startTime = System.nanoTime();
        long durationNanoseconds = durationMicroseconds * 1000;
        while (System.nanoTime() < startTime + durationNanoseconds) {
            if (mGpio.getValue() != level) {
                pulse = true;
                break;
            }
            try {
